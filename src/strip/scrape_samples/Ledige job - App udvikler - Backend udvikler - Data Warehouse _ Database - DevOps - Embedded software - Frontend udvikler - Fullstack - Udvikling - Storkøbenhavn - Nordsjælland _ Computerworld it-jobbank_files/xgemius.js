...
if (typeof GemiusHitcolConnector == "undefined") {
	GemiusHitcolConnector = function(is_gde) {
		...
		this.ref = {ref: "", frametype: 0, topref: 0};
		...
		if (this.use_globals) {
			...
			this.hc = (typeof gemius_hitcollector === 'string')?gemius_hitcollector:(typeof pp_gemius_hitcollector === 'string')?pp_gemius_hitcollector:'gadk.hit.gemius.pl';
		...
		this.send_bfview = function() {
			...
			if (bfview!=null) {
				this.internal_hit(0,bfview.vers,bfview.id,bfview.evid,"bfview",bfview.hsrc,0,bfview.extra);
			}
		};
		...
		this.internal_hit = function(allowaddscript,vers,id,evid,et,hsrc,sonar,extra) {
			var req = "";
			...
			req += '&eventid='+evid+this.parameters();
			var robj = {req:req,id:id,allowaddscript:allowaddscript,vers:vers,explicit:this.explicit_consent};
		...
		this.tfp_send_fixes = function(unload) {
			...
			for (var id in this.tfp.fixes) {
				...
				var has_new_fix = ((fix.fpdata && has_fpdata) || (fix.tfpdata && has_tfpdata) || (fix.ref && has_ref));
...
if (typeof gemius_hcconn == "undefined") {
	gemius_hcconn = new GemiusHitcolConnector(false);
	gemius_hcconn.start();
}
